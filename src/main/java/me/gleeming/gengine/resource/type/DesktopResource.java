...
public class DesktopResource extends Resource {
    ...
    public DesktopResource(String path) { super(path); if(!path.startsWith("/")) path = "/" + path; try { this.bufferedImage = ImageIO.read(Gengine.getInstance().getClass().getResource(path)); } catch(Exception ex) { ex.printStackTrace(); System.out.println("[Gengine] There was an error while attempting to read a resource from '" + path + "'"); } setDesktopResource(this); }
